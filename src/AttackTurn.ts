...
const affichageResultat = document.getElementById(
  "resultCombat",
) as HTMLElement | null;
...
export function ShowTurn(initiativeTour, turnNumber) {
  ...
  if (player.id === 6) {
    ...
    const objetline = document.getElementById(
      "objetsdispos",
    ) as HTMLElement | null;
    ...
    if (result >= troglo.dc) {
      ...
      if (troglo.hp <= 0) {
        ...
        if (temps <= 1) {
          ...
          reussite.className =
            "d-flex justify-content-center align-items-center vh-100 text-center h4";
          reussite.innerHTML = `
          <div>
...
function trogloAttack() {
  ...
  const alive = Object.values(party).filter(
    (slot: any) =>
